...
public class CreditCardAdapter extends RecyclerSwipeAdapter<CreditCardAdapter.CustomHolder> {
    ArrayList<CreditCard> list;
    Context context;

    public CreditCardAdapter(Context context, ArrayList<CreditCard> list) {
        this.context = context;
        this.list = list;
    }

    @Override
    public CreditCardAdapter.CustomHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        return new CustomHolder(LayoutInflater.from(context).inflate(R.layout.creditcardetails_item_list, parent, false));

    }

    @Override
    public void onBindViewHolder(CreditCardAdapter.CustomHolder holder, final int position) {
        ...
        String c_bal = String.valueOf(list.get(position).getAvailablelimit());
        if (!c_bal.equalsIgnoreCase("") && c_bal != null) {
            holder.availableBalanceText.setText("Available Bal : " + "\u20B9 " + c_bal);
        } else {
            holder.availableBalanceText.setText("Available Bal : " + "\u20B9 " + "00.00");
        }
        String credit = list.get(position).getPaymentmade().getAmount();
        if (!credit.equalsIgnoreCase("") && credit != null) {
            holder.creditamountTextview.setText("Credit : " + "\u20B9 " + credit);
        } else {
            holder.creditamountTextview.setText("Credit : " + "\u20B9 " + "0.0");
        }
        String atm = list.get(position).getPaymentmade().getDesc();
        if (!atm.equalsIgnoreCase("") && atm != null) {
            holder.creditdesTextview.setText(atm);
        } else {
            holder.creditdesTextview.setText("--");
        }
       /* String transfer = list.get(position).getPaymentmade().getDesc();
            ...
            public void onClick(View v) {
                ...
                alertbox.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(final DialogInterface dialog, int which) {
                        WebServices.getInstance(context, "").deleteAccount(ConstantValues.DELETEACCOUNT, Session.getInstance(context, "").getUserid(), Session.getInstance(context, "").getToken(), String.valueOf(list.get(position).getBankid()), new VolleyResponseListerner() {
                            @Override
    ...
    public class CustomHolder extends RecyclerView.ViewHolder {
        ...
        TextView accountNotext, availableBalanceText, creditamountTextview, creditdesTextview;

        public CustomHolder(View itemView) {
            super(itemView);
            accountNotext = (TextView) itemView.findViewById(R.id.accountNotext);
            availableBalanceText = (TextView) itemView.findViewById(R.id.availableBalanceText);
            creditamountTextview = (TextView) itemView.findViewById(R.id.creditamountTextview);
            creditdesTextview = (TextView) itemView.findViewById(R.id.creditdesTextview);
